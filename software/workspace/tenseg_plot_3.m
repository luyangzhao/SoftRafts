function [fig_out] = tenseg_plot_3( N,C_b,C_s,fig_handle,highlight_nodes,view_vec, PlotTitle, R3Ddata)
...
C=[C_b;C_s];
index_b1=1:size(N,2)/2;   % index of bars
index_b2=size(N,2)/2+1:size(C_b,1)-size(C_s,1); 
index_s=(size(C_b,1)+1):(size(C_b,1)+size(C_s,1)); % index of strings
C_b1 = C_b(index_b1,:);  C_b2 = C_b(index_b2,:);
...
if ~isempty(C_b1)
    ...
    for j = 1:size(B,2)
        bar_start_nodes(:,j) = N(:,C_b1(j,:)==-1);
        bar_end_nodes(:,j) = N(:,C_b1(j,:)==1);
    end
...
if ~isempty(C_b2)
    ...
    bar_end_nodes = zeros(3,size(B,2));
    for j = 1:size(B,2)
        bar_start_nodes(:,j) = N(:,C_b2(j,:)==-1);
        bar_end_nodes(:,j) = N(:,C_b2(j,:)==1);
    end
     quiver3(bar_start_nodes(1,:),bar_start_nodes(2,:),bar_start_nodes(3,:),B(1,:),B(2,:),B(3,:),'red.','Autoscale','off','LineWidth',BarWidth)
     hold on
...
if ~isempty(C_s)
    ...
    if ~isempty(R3Ddata) && isfield(R3Ddata,'Sradius') % 3D plot
        ...
        quiver3(string_start_nodes(1,:),string_start_nodes(2,:),string_start_nodes(3,:),S(1,:),S(2,:),S(3,:),'magenta.','Autoscale','off','LineWidth',StringWidth);
        hold on
    end
