function [fig_out] = tenseg_plot_process( N,C_b,C_s,fig_handle,highlight_nodes,view_vec, PlotTitle, R3Ddata)
...
if ~isempty(C_b)
    ...
    if ~isempty(R3Ddata) && isfield(R3Ddata,'Bradius') % 3D plot
        ...
        quiver3(bar_start_nodes(1,:),bar_start_nodes(2,:),bar_start_nodes(3,:),B(1,:),B(2,:),B(3,:),'black.','Autoscale','off','LineWidth',BarWidth,'Linestyle','--')
        hold on
    end
    % Write bar labels
    if LabelBars == 1
